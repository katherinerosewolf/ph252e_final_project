...
get.tab1 <- function(df = as.data.table(as.data.frame(frm))) {
...
tab1 <- get.tab1()
tab1.stroke <- get.tab1(df = as.data.table(filter(as.data.frame(frm), stroke == 1)))

cbind(tab1, " " = NA, tab1.stroke) %>% xtable(
	align = "llllll") %>% print(
		include.rownames = T,
		hline.after =  c(0, grep("Years of follow-up", rownames(.)) - 1, nrow(.)),
		add.to.row = list(
			pos = list(-1,
								 grep("Years of follow-up", rownames(.)) - 1
								 # nrow(.)
			),
			command = c(
				"\\toprule & \\multicolumn{2}{c}{Full cohort} & \\multicolumn{3}{c}{Stroke cases}\\\\ \\midrule",
